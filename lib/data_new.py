...
def get_camera_matrices1(metadata_filename):
...
class RGBA2SDF(torch.utils.data.Dataset):
    def __init__(
        ...
        # self.npyfiles =  get_instance_filenames(data_source, split)
        self.data_path = os.path.join(self.data_source, self.class_id)
        self.sketch_path_source = '/public2/home/huyuanqi/data/shapenet-sketch'
        self.folder_names = sorted(os.listdir(self.data_path))
        self.result_path = '/public2/home/huyuanqi/project/MeshSDF_hecheng/experiments/watercraft_new/Reconstructions/latest'
        self.r_names = sorted(os.listdir(self.result_path))
        for f in self.r_names:
            self.folder_names.remove(f)
    ...
    def __getitem__(self, idx):
        ...
        image_filename = os.path.join(self.data_source, self.class_id, mesh_name, 'sketch.png')
        ...
        metadata_filename = os.path.join(self.data_source, self.class_id, mesh_name, 'view.txt')
        intrinsic, extrinsic = get_camera_matrices1(metadata_filename)
